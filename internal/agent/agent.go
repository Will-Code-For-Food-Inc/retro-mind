...
// ChatMessage is a single message in an Ollama chat history.
type ChatMessage struct {
	Role    string `json:"role"`
	Content string `json:"content"`
}

// Agent wraps an Ollama chat endpoint for the vibe-tagging agent loop.
type Agent struct {
	baseURL      string
	model        string
	systemPrompt string
...
// New creates and returns an Agent, reading configuration from environment
// variables and optionally loading a prompt override from dataDir.
func New(dataDir string) (*Agent, error) {
...
func (a *Agent) Generate(messages []ChatMessage) (string, error) {
...
// RunAgentLoop drives the tool-calling agent loop. The caller injects toolsJSON
// (which returns the JSON schema of available tools) and callTool (which
// dispatches a tool call and returns its result plus an error flag).
func (a *Agent) RunAgentLoop(
	prompt string,
	toolsJSON func() string,
