...
func (c *ollamaClient) chat(messages []ollamaChatMessage, tools []ollamaTool) (ollamaChatMessage, error) {
	...
	if db.DB != nil {
		db.DB.Exec(`INSERT INTO llm_metrics (model, prompt_tokens, completion_tokens, total_ms, prompt_ms, gen_ms, tok_per_sec, tool_name)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
func statusCmd(mcp *mcpClient) tea.Cmd {
	return func() tea.Msg {
		...
		if db.DB != nil {
			db.DB.QueryRow("SELECT COUNT(*), COALESCE(AVG(tok_per_sec), 0) FROM llm_metrics").Scan(&totalCalls, &avgTokSec)
		}
		if totalCalls > 0 {
...
func (v *agentView) metricsCmd() tea.Cmd {
	var lines []string
	if db.DB != nil {
		rows, err := db.DB.Query(`SELECT model, COUNT(*), SUM(prompt_tokens), SUM(completion_tokens),
