...
func (c *mcpClient) ensureStarted() error {
	...
	bin, err := resolveRetroMindBinary()
	if err != nil {
	...
	if _, err := c.requestLocked("initialize", map[string]interface{}{
		...
		"clientInfo":      map[string]string{"name": "retro-mind-tui", "version": "0.1.0"},
	}); err != nil {
...
func resolveRetroMindBinary() (string, error) {
	if path := os.Getenv("ROM_TAGGER_MCP_BIN"); path != "" {
		return path, nil
	}
	// Try retro-mind first, then legacy names.
	for _, name := range []string{"retro-mind", "rom-tagger-mcp", "rom-tagger"} {
	...
	for _, root := range searchRoots {
		for _, name := range []string{"retro-mind", "rom-tagger"} {
	...
	return "", fmt.Errorf("could not find retro-mind MCP binary; set ROM_TAGGER_MCP_BIN or put retro-mind on PATH")
}
